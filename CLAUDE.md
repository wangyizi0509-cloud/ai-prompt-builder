...
3. **项目变更记录**：`ITERATION_LOG.md` 只记录仍有接力价值的项目级事实，包括跨模块改造、接口/契约变化、部署问题、产品/工程决策和未完成事项。不要把日常小修、一次性跑测、临时调试过程写成流水账；这些信息放在当前对话、commit/PR、artifact 或 memory 中。
   - **更新方式**：只有确实需要后续 agent 或 PM 接手时才更新 `ITERATION_LOG.md`。新增记录必须精简，写清结论、状态、影响范围、验证方式和下一步；已闭环事项只留摘要，未闭环事项必须写清待办。
...
旧的 `onboarding` 子图（`agent_impl/onboarding/`）仍被挂在 workflow 上作为兜底，但 router 只要发现 `go_onboarding=True` 就会打 warning（`graph/nodes/router.py:250-258`）——正常路径不应触发。新用户走的是下一节的 Onboarding v2 独立通道。
...
- `agent_impl/context_system/` 与 `context_system_v2/`：分层长期记忆的提取策略（layer1/2/3 从对话中抽取/更新的规则与 prompt）。主图节点调用这里的 extractor 来更新 `AgentState` 各层记忆。
- `agent_impl/strategies/`：LLM 模型选择与工具使用策略配置（per-node 模型、温度、可用工具白名单等）。
- `agent_impl/agents/`：可被主图节点或子图复用的 agent 构造器与共享工具逻辑（与 `graph/subgraphs/` 的"LangGraph 子图"概念不同）。
...
**实际上线流程：`wyz_1` 开发完成后，merge 到 `develop` 并 push，即触发线上部署。`develop` 就是当前生产环境。`main` 暂不使用。**

- `develop`：**生产分支**，push 后自动触发 LangChain Cloud + Zeabur 部署
- `wyz_1`：日常开发主分支，完成后 merge 到 develop 上线
- `feature/*` / `fix/*`：从 `wyz_1` 拉出，完成后 PR 回 `wyz_1`
- `main`：暂不使用，留作将来规范化备用
