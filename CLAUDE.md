...
> 注：CI/CD 触发逻辑仍按下面的「理想模型」配置（`.github/workflows/ci.yml` 认 `main` / `develop`），但实际开发流已迁移到 `wyz_1` 为主工作分支。日常 feature 直接基于 `wyz_1` 开分支、PR 回 `wyz_1`；`develop` 作为历史参考保留。发布时才走 `wyz_1 → develop → main` 汇流。

- `main`：生产基线，触发自动 CD，**只接受来自 `develop` 的 PR**，严禁直接 commit
- `develop`：集成分支（当前阶段主要作为历史参考，非日常合入点）
- `wyz_1`：当前日常开发主分支
