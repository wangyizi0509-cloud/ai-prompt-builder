...
| `append_task_note(note)` | 记录关键推理/决策（追加到当前任务） | 记录重要判断供后续参考 |
...
当上下文中只看到某条行动指南的摘要/元数据（表格里有 ID），但需要**持续参考**该指南的完整内容时：

| 工具 | 何时使用 | 效果 |
|------|----------|------|
| `bind_action_guide_detail(guide_id)` | 需要跨轮次持续查看某条指南的完整内容 | 绑定后，后续轮次自动注入该指南详情 |

**使用场景**：
*   执行某个任务时，需要反复参考特定指南的步骤和话术
*   切换任务后，绑定的详情会自动隐藏；切回时自动恢复

**限制**：每个任务最多绑定 3 篇指南，超出时自动淘汰最早绑定的。
