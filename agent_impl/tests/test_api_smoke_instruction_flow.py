...
def test_main_to_status_instruction_smoke():
    ...
    # 通过强约束提示，尽量让 Main Agent 稳定输出 call_status + instruction
    state = create_initial_state(
        "你现在是 Main Agent。请严格只输出 JSON，不要输出多余文本。\n"
        "要求：\n"
        '- next_action 必须为 "call_status"\n'
    ...
    # 允许 Main Agent 在真实场景下先 ask_user 一轮；但由于 max_questions=1，补充一次信息后应进入 call_status
    if out1.get("next_action") == "ask_user":
    ...
    # 如果此时还停在 call_status，继续执行到 Status Agent（真实 API）。
    # 注意：在某些路径下，run_workflow_turn 可能已经把 status_agent 执行完并返回 end_turn（等待用户回答）。
    if out1.get("next_action") == "call_status":
        out2 = wf.invoke(out1, config={"configurable": {"thread_id": "api_smoke_instruction"}})
    else:
        out2 = out1
    ...
    status_ran = (
        ...
        or out2.get("inquiry_card") is not None
        or bool(out2.get("pending_questions"))
    )
    assert status_ran, f"未观察到 Status Agent 的执行结果（completion/status_report/inquiry_card/pending_questions），next_action={out2.get('next_action')}"
