...
"""
    ...
    if from_tool_call and not last_tool_content:
        # 兜底：如果最后一条不是 tool 消息（或内容为空），回溯查找最近一次 tool 输出
        for msg in reversed(messages or []):
            r, c = get_msg_role_and_content(msg)
            if r == "tool" and c:
                last_tool_content = c
                break
        # 进一步兜底：从 skill_tools 节点缓存的工具输出中读取
        cached_tool_outputs = state.get("_last_tool_outputs") if isinstance(state, dict) else None
        if not last_tool_content and isinstance(cached_tool_outputs, list):
