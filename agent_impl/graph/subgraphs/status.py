...
def llm_step_node(state: StatusSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def tool_step_node(state: StatusSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
    task_spec = state.get("task_spec") if isinstance(state.get("task_spec"), dict) else {}
    tools = _build_status_tools(task_spec)
...
def create_status_subgraph() -> StateGraph:
    ...
    def _route_after_llm(state: StatusSubState) -> str:
