...
class GuideSubState(TypedDict, total=False):
    task_spec: dict
...
def _build_guide_tools(task_spec: dict) -> list[BaseTool]:
    parent_state = task_spec.get("parent_state") if isinstance(task_spec.get("parent_state"), dict) else {}

    def state_getter() -> dict:
        return parent_state

    return replace_ask_human_tool(build_all_tools_for_agent("guide", state_getter=state_getter))
...
def _create_inner_agent(
    ...
    cfg = build_inner_agent_config(config, namespace="guide_tool_loop")
...
def run_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def handle_interrupt_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def finalize_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def cleanup_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def create_guide_subgraph() -> StateGraph:
    builder = StateGraph(GuideSubState)
...
def get_guide_subgraph(checkpointer: Optional[BaseCheckpointSaver] = None):
    key = id(checkpointer) if checkpointer is not None else 0
    cached = _compiled_cache.get(key)
    if cached is not None:
        return cached
    builder = create_guide_subgraph()
