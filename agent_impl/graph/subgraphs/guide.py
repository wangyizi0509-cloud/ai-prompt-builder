...
def llm_step_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
...
def tool_step_node(state: GuideSubState, config: RunnableConfig | None = None) -> dict[str, Any]:
    task_spec = state.get("task_spec") if isinstance(state.get("task_spec"), dict) else {}
    tools = _build_guide_tools(task_spec)
...
def create_guide_subgraph() -> StateGraph:
    ...
    def _route_after_llm(state: GuideSubState) -> str:
